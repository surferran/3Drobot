...
// READ IN THE LIST OF CHESSBOARDS:
    ...
    //FIND CHESSBOARDS AND CORNERS THEREIN:
        ...
        if( displayCorners )
        {
            ...
            cvReleaseImage( &cimg );
			//if (cvWaitKey(0) == 27) //Allow ESC to quit  //RAN
            if( cvWaitKey(10) == 27 ) //Allow ESC to quit
                exit(-1);
        }
        else
            putchar('.');
...
// HARVEST CHESSBOARD 3D OBJECT POINT LIST:
	...
	///CvMat _imagePoints1,  _imagePoints2;
	//if (calibrateOnlyRight == false ) /*CvMat*/ _imagePoints1 = cvMat(1, N, CV_32FC2, &points[0][0] );  //RAN
	///if (calibrateOnlyLeft  == false) /*CvMat*/  _imagePoints2 = cvMat(1, N, CV_32FC2, &points[1][0] );  //RAN
...
int main_PushBroom(int argc, char *argv[])
{
    int nx, ny;
    float squareSize;
    int fail = 0;
